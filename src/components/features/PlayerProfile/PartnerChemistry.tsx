...
export function PartnerChemistry({ playerId }: PartnerChemistryProps) {
    ...
    return (
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        ...
        <div className="text-center py-6">
          ...
          <p className="text-wefit-grey text-xs mt-1">Play doubles to build chemistry!</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-6">
        ...
        {partners.slice(0, 5).map((partnerData, index) => {
          ...
          return (
            <div
              ...
              className="bg-gray-50 border border-gray-200 rounded-lg p-4 hover:border-wefit-primary/40 transition-all group"
            >
              <div className="flex items-center gap-3">
                ...
                {partnerData.partner.avatar_url ? (
                  <img
                    ...
                    className="w-12 h-12 rounded-full object-cover ring-2 ring-gray-200 group-hover:ring-cyan-500 transition-colors"
                  />
                ) : (
                  <div className="w-12 h-12 rounded-full bg-gradient-to-br from-cyan-500 to-blue-500 flex items-center justify-center text-white font-bold text-sm ring-2 ring-gray-200 group-hover:ring-cyan-500 transition-colors">
                    {initials}
                  </div>
                )}
      ...
      {partners.length > 5 && (
        <button className="w-full mt-4 px-4 py-2 border border-wefit-primary text-wefit-primary hover:bg-wefit-primary/10 rounded-lg transition-colors text-sm">
