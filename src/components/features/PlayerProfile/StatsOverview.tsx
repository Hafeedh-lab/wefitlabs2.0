...
export function StatsOverview({ profile, stats }: StatsOverviewProps) {
  if (!stats || stats.matches_played === 0) {
    return (
      <div className="mt-6 bg-wefit-dark-muted/50 border border-white/5 rounded-xl p-8 text-center">
        <Target className="w-16 h-16 text-wefit-grey/60 mx-auto mb-4" />
        <h3 className="text-xl font-semibold text-wefit-grey mb-2">No Matches Yet</h3>
        <p className="text-wefit-grey/80">Play your first match to see your stats!</p>
      </div>
    );
  }
  ...
  return (
    <div className="mt-6 space-y-6">
      {/* Performance Trend */}
      <div className="bg-gradient-to-r from-wefit-dark to-wefit-dark-muted border border-white/5 rounded-xl p-6">
        <h3 className="text-lg font-semibold text-wefit-white mb-4">Performance Trend</h3>
        <div className="flex items-center gap-4">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-wefit-grey capitalize">{trend}</span>
            </div>
            <p className="text-sm text-wefit-grey">
              Average point differential:{' '}
              <span className={avgPointDiff > 0 ? 'text-wefit-success' : avgPointDiff < 0 ? 'text-wefit-error' : 'text-wefit-grey'}>
          ...
          <div className="text-right">
            <div className="text-3xl font-bold text-wefit-white">{winRate}%</div>
        ...
        {/* Win/Loss Bar */}
        <div className="mt-4">
          <div className="h-3 bg-wefit-dark-muted rounded-full overflow-hidden">
            <div
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {statCards.map((stat) => (
          <div
            key={stat.label}
            className="bg-wefit-dark-muted/70 border border-white/5 rounded-lg p-4 hover:border-wefit-primary/40 transition-colors"
          >
            <div className="flex items-center justify-between mb-2">
              <stat.icon className={`w-5 h-5 ${stat.color}`} />
            </div>
            <div className="text-2xl font-bold text-wefit-white mb-1">{stat.value}</div>
      ...
      {stats.last_played_at && (
        <div className="bg-wefit-dark-muted/50 border border-white/5 rounded-lg p-4 text-center">
          <p className="text-sm text-wefit-grey">
            Last played:{' '}
            <span className="text-wefit-white font-medium">
